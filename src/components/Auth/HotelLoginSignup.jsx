...
function HotelLoginSignup() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) {
      alert(`${isSignUp ? 'Hotel account created' : 'Hotel login successful'}`);
    }
  };
      ...
      <div className={`auth-container ${isSignUp ? 'signup-mode' : ''}`}>
        <div className="form-box">
          <form onSubmit={handleSubmit}>
            <h2>{isSignUp ? 'Hotel Sign Up' : 'Hotel Login'}</h2>
            ...
            <button type="submit" className="submit-btn">
              {isSignUp ? 'Hotel Sign Up' : 'Hotel Log In'}
            </button>

            <p className="switch-text">
              {isSignUp ? 'Already have a hotel account?' : "Don't have a hotel account?"}
